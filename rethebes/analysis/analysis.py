...
def plot_temp_vs_load(data, axis):
    ...
    axis.plot(data["Load CPU Total"].rolling(window).mean(), data["Temperature Core Average"].rolling(window).mean(), color = "g", marker = ".", linestyle = "none", label = "Avg")
    axis.plot(data["Load CPU Total"].rolling(window).mean(), data["Temperature Core Max"].rolling(window).mean(), color = "r", marker = ".", linestyle = "none", label = "Max")
...
def plot_time_load_temp(data, axis):
    delta_t = data["Time"].diff().mean()/np.timedelta64(1, "s")
    window = int(10 / delta_t)
    axis.plot(data["Time"], data["Load CPU Total"].rolling(window).mean(), color = "0.5", marker = ".", linestyle = "none", label = "Load")
    axistwin = axis.twinx()
    axistwin.plot(data["Time"], data["Temperature Core Average"].rolling(window).mean(), color = "g", marker = ".", linestyle = "none", label = "Temp. Avg")
    axistwin.plot(data["Time"], data["Temperature Core Max"].rolling(window).mean(), color = "r", marker = ".", linestyle = "none", label = "Temp. Max")
    axis.autoscale(tight = True)
    axis.grid()
    axis.legend()
    axis.xaxis.set_major_formatter(date_format)
